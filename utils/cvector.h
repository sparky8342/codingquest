 ...
 * @internal
 */
#define cvector_vec_to_base(vec) (&((cvector_metadata_t *)(void *)(vec))[-1])
    ...
    do {                                                                       \
        if (vec) {                                                             \
            ...
            if (cv_clear_elem_dtor__) {                                        \
                size_t cv_clear_i__;                                           \
                for (cv_clear_i__ = 0; cv_clear_i__ < cvector_size(vec);       \
                     ++cv_clear_i__) {                                         \
    ...
    do {                                                                       \
        if (vec) {                                                             \
            ...
            if (cv_free_elem_dtor__) {                                         \
                size_t cv_free_i__;                                            \
                for (cv_free_i__ = 0; cv_free_i__ < cvector_size(vec);         \
                     ++cv_free_i__) {                                          \
...
#define cvector_at(vec, n)                                                     \
    ((vec) ? (((int)(n) < 0 || (size_t)(n) >= cvector_size(vec)) ? NULL        \
                                                                 : &(vec)[n])  \
           : NULL)
...
#define cvector_back(vec)                                                      \
    ((vec) ? ((cvector_size(vec) > 0) ? cvector_at(vec, cvector_size(vec) - 1) \
                                      : NULL)                                  \
           : NULL)
